...
void registerCommand(char *cmd, char delimeter, cmd_handler handler, char *help)
{
	...
	//later we let python know that this command handler is located at 
	//this index and thus python can just pass us this index instead of a 
	//command string which will make finding the command handler quicker
	...
	cmd_list[CURRENT_NUM_OF_COMMANDS].index			= CURRENT_NUM_OF_COMMANDS;
...
{ 
	...
	for (i = 0; i < CURRENT_NUM_OF_COMMANDS; i++)
	{
		...
		{   
			...
			//CL_printMsg("\r\n"); this not needed in python version
...
{
	/*	send some sort of delimeter to python
		it should identify this delimeter as a
		"start" of packet string indicatin that
		the following data is a list of commands
		and their corrsponding indexs        */

	//send start of packet delimeter

	//cycle throught cmd_list, send cmd string and index
